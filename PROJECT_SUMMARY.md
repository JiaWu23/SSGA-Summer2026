...
The current best production-like interpretation is the **long-only** sleeve. Long/short is still run for research comparison, but shorts have generally hurt results in this index sleeve universe. The headline metrics below come from the same checked-in long-only backtest outputs used by the final report.

| Window | M1 Only | M1 + M2 + M3 Binary | M1 + M2 + M3 ECDF |
| --- | --- | --- | --- |
| Full sample (train + test) | 5.91% ann. return / 0.617 Sharpe / -20.17% max drawdown | 5.66% / 0.618 / -18.21% | 1.81% / 0.332 / -13.11% |
| Test period (2021+) | 8.92% / 0.875 / -20.17% | 8.67% / 0.951 / -14.45% | 4.58% / 0.914 / -7.51% |

Main takeaway: **M1 remains the primary return-oriented engine.** On the current 2021+ test window, M3 binary and ECDF materially improve Sharpe and drawdown versus M1-only, but the evidence for stable walk-forward improvement is mixed. The current checked-in walk-forward analysis reports a mean ECDF Sharpe edge of -0.190 with positive results in only 1 of 6 folds.

Reviewer caveat: the equal-weight benchmark is shown with 0 bps transaction costs, while strategy variants pay the configured 5 bps turnover cost. M1's average gross exposure is about 81%, not 100%, so the result should be framed as similar return with lower risk and drawdown rather than as strong positive excess return.
...
Crucially, M2 never creates a trade by itself. It only scores candidate trades generated by M1. When M1 is flat (`M1_signal = 0`), the M2 panel rows are absent or masked for training and no M2-based position can be created.
...
It only acts on candidate trades that already have a non-zero M1 signal. When M1 is flat, the allocation state is `no_signal` and M3 has no opportunity to initiate a position.
